...
import modellingcore.impl.AbstractRelationshipImpl;
	...
	public ActivityPartition getInActivityPartition() {
		if (inActivityPartition != null && inActivityPartition.eIsProxy()) {
			...
			if (inActivityPartition != oldInActivityPartition) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, ActivityPackage.ACTIVITY_EDGE__IN_ACTIVITY_PARTITION, oldInActivityPartition, inActivityPartition));
			}
		}
	...
	public InterruptibleActivityRegion getInInterruptibleRegion() {
		if (inInterruptibleRegion != null && inInterruptibleRegion.eIsProxy()) {
			...
			if (inInterruptibleRegion != oldInInterruptibleRegion) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, ActivityPackage.ACTIVITY_EDGE__IN_INTERRUPTIBLE_REGION, oldInInterruptibleRegion, inInterruptibleRegion));
			}
		}
	...
	public StructuredActivityNode getInStructuredNode() {
		if (inStructuredNode != null && inStructuredNode.eIsProxy()) {
			...
			if (inStructuredNode != oldInStructuredNode) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, ActivityPackage.ACTIVITY_EDGE__IN_STRUCTURED_NODE, oldInStructuredNode, inStructuredNode));
			}
		}
