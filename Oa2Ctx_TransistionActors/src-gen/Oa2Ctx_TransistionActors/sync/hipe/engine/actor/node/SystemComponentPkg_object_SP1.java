...
public class SystemComponentPkg_object_SP1 extends GenericObjectActor<org.polarsys.capella.core.data.ctx.SystemComponentPkg> {
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, ObjectNode node) {
		...
		ports.add(new PortNodeMatch<org.polarsys.capella.core.data.ctx.SystemComponentPkg>(node.getPorts().getPort().get(1), getSelf(), name2actor.get("Oa2Sa_transition_actors__CONSISTENCY_37_localSearch"), this::returnTrue   , node.getName() ));
		ports.add(new PortNodeLeft<org.polarsys.capella.core.data.ctx.SystemComponentPkg>(node.getPorts().getPort().get(2), getSelf(), name2actor.get("SystemComponentPkg_ownedParts_0_reference"), this::check_constraint_11 , 0   ));
	}
