...
public class Oa2Sa_generation__SRC_32_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("Oa2Sa_generation__FWD_35_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("Oa2Sa_generation__SRC_production"), this::returnTrue , 0  , false ));
	}
			...
			case "OperationalAnalysis_object_SP0": 
				{
					...
					// src_oa_arch
					HMatch match_0 = new LocalSearchMatch("Oa2Sa_generation__SRC_32_localSearch", 2);
			...
			case "EntityPkg_object_SP0": 
				{
					if(!lazy_initialization) {
						...
						// src_entity_pkg
						HMatch match_1 = new LocalSearchMatch("Oa2Sa_generation__SRC_32_localSearch", 2);
						match_1.getNodes()[1] = objs[0];
						start(edge_explorer_2_orchestration, match_1);
					}
				}
				break;
			case "Oa2Sa_generation_src_oa_arch_ownedEntityPkg_outgoing_SRC__FILTER_NAC_SRC_11_localSearch": 
				{
		...
		switch(match.creator) {
			...
			case "Oa2Sa_generation_src_oa_arch_ownedEntityPkg_outgoing_SRC__FILTER_NAC_SRC_11_localSearch": 
				if(nac_checker.deregister(match)) {
					HMatch acMatch_0 = new LocalSearchMatch("Oa2Sa_generation__SRC_32_localSearch", 2);
					Object[] acMatch_nodes_0 = acMatch_0.getNodes();
	...
	protected void addReference(ReferenceAdded msg) {
		...
		case "org.polarsys.capella.core.data.oa.OperationalAnalysis_ownedEntityPkg_EntityPkg": 
			if(!lazy_initialization) {
				{
					HMatch match = new LocalSearchMatch("Oa2Sa_generation__SRC_32_localSearch", 2);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					currentDepth++;
