...
public class Oa2Sa_generation__FWD_35_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("Oa2Sa_generation__CONSISTENCY_36_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("Oa2Sa_generation__FWD_production"), this::returnTrue , 0  , false ));
	}
	...
	protected void added(MatchAdded<HMatch> msg) {
		...
		switch(match.creator) {
			case "Oa2Sa_generation__SRC_32_localSearch": 
				{
					if(pac_explorer.register(match)) {
						HMatch acMatch_0 = new LocalSearchMatch("Oa2Sa_generation__FWD_35_localSearch", 2);
	...
	protected void registerMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// register Oa2Sa_generation__SRC
	...
	protected void deregisterMatch(HMatch match) {
		...
		Object[] objs = match.getNodes();
		// deregister Oa2Sa_generation__SRC
		OverlapMatch oMatch_0 = new OverlapMatch(2);
		Object[] oMatch_Nodes_0 = oMatch_0.getNodes();
		oMatch_Nodes_0[0] = objs[0];
		oMatch_Nodes_0[1] = objs[1];
	...
	protected void changeAttribute(AttributeChanged message) {
		...
		if(obj instanceof org.polarsys.capella.core.data.oa.OperationalAnalysis) {
			HMatch match_1 = new LocalSearchMatch("Oa2Sa_generation__FWD_35_localSearch", 2);
		...
		if(obj instanceof org.polarsys.capella.core.data.oa.EntityPkg) {
			HMatch match_0 = new LocalSearchMatch("Oa2Sa_generation__FWD_35_localSearch", 2);
